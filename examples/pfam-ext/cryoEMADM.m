...
% date: 7/14, 2012
%
% Modified by: Haoyang Liu, April 22, 2019
% Reference: Wang Lanhui, Amit Singer, Wen Zaiwen; Orientation 
%   Determination from Cryo-EM images Using Least Unsquared Deviation;
%   SIAM Journal on Imaging Sciences; vol. 6, no.4 (2013), 2450-2483
%------------------------------------------------------------------
...
for itr = 1:maxit
    ...
    elseif adp_proj == 2
        if itr == 1
            ...
            workproj = struct();
            workproj.degree = 5;
            workproj.force_exact = 2;
            workproj.k = -1;
        end
        % computation of W and H
        [V,dH, workproj] = eig_inexact(-H, workproj);
        %dH = diag(dH);
        nD = dH>EPS; nev = nnz(nD); 
        %dH'
        if nev > 0
            dH = dH(nD);
            WmH = V(:,nD)*diag(dH)*V(:,nD)';
